...
import { IPack } from '../[code]/_types/code.type';
    ...
    return (
        <div className="card">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 md:gap-2 lg:gap-3 xl:gap-4">
                {
                    packs.map((pack, i) => {
                        const urlImage = `/api/image-proxy?url=${process.env.NEXT_PUBLIC_URL_S3}/${pack.code}/${pack.pages[0].page_size[0].size.name}/${pack.pages[0].num}.${pack.pages[0].page_size[0].size.extension}`
                        //const urlImage = `${process.env.URL_S3}/${pack.code}/${pack.pages[0].page_size[0].size.name}/${pack.pages[0].num}.${pack.pages[0].page_size[0].size.extension}`
                        return (
                            <Card
                                code={pack.code}
