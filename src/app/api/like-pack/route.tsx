...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const verified = await repositoryLikePack.verfiedLikeForUser(user!.user_id, body.pack_id)
        logger.info(`api/like POST verified ${jsonLog(verified)} `)
        if (verified === null) {
            await repositoryLikePack.create(body.pack_id, user.user_id);
        } else {
            await repositoryLikePack.delete(user.user_id, body.pack_id);
        }
